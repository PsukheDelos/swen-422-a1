...
Leap.loop(controllerOptions, function (frame) {
    ...
    // try incase leap cannot be found or hand(s) cannot be found
    try {

        // variables relating to each loop
        ...
        // calculate cursor position in this frame
        ...
        // the following if/else if chain looks for different states of selection to interpret relevant gestures
        if (frame.pointables.length > 0 && frame.hands.length == 1) {
            if (touchZone != "touching" && $('#selected').size() == 0) {
                ...
                var cssObj = {
                    'left': appX - $('#selected').width() - 40,
                    'top': (appY - $('#selected').height() / 2)
                };
                $('#selected').css(cssObj);
                $('#selected').css({
                    background: "red"
